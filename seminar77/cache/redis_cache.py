...
class RedisCache:
    def __init__(self, config: dict):
        ...
        def set_value(self, name: str, value_dict: dict, ttl: float):
            value_js = json.dumps(value_dict)    #переставляем словарь в json
            try:
        ...
        def get_value(self, name):
            value_js = self.conn.get(name) # получили данные в формате json
            if value_js:
                value_dict = json.loads(value_js) # переставляем json в словарь
                return value_dict
            else:
                return None